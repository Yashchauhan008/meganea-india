...
import { 
    X, Loader2, Edit, Save, Warehouse, Box, Ruler, Truck, Package,
    Calendar, User, FileText, RotateCcw, CheckCircle, AlertCircle,
    Ship, MapPin, Clock, Boxes, FileSpreadsheet
} from 'lucide-react';
import { updateContainer, updateContainerStatus } from '../../api/containerApi';
import PalletSelectionModal from '../loading-plans/PalletSelectionModal';
import Input from '../ui/Input';
...
const ContainerDetailModal = ({ container: initialContainer, onClose, onUpdate }) => {
    ...
    const handleExportPDF = async () => {
        ...
        await generatePDFReport({
            ...
            subtitle: `Status: ${container.status} | Factory: ${factoryName}`,
            headerInfo: [
                { label: 'Container ID', value: container.containerId },
                { label: 'Container No.', value: container.containerNumber || 'N/A' },
                { label: 'Truck No.', value: container.truckNumber || 'N/A' },
                { label: 'Status', value: container.status },
                { label: 'Factory', value: factoryName },
                { label: 'Loading Plan', value: container.loadingPlan?.planId || 'N/A' },
                { label: 'Created By', value: container.createdBy?.username || 'N/A' },
                { label: 'Created Date', value: container.createdAt ? format(new Date(container.createdAt), 'dd MMM, yyyy') : 'N/A' },
            ],
            summaryData: [
                { label: 'Total Pallets', value: totalPalletCount, color: 'blue' },
                { label: 'Total Khatlis', value: totalKhatliCount, color: 'purple' },
            ...
            tableColumns: [
                ...
                { key: 'factory', header: 'Factory', width: 40 },
            ],
            tableData: allPallets.map((pallet, idx) => ({
                sNo: idx + 1,
                ...
                factory: pallet?.factory?.name || '-',
            })),
            fileName: `Container_${container.containerId}`,
    ...
    const handleExportExcel = async () => {
        ...
        await generateExcelReport({
            ...
            summaryData: [
                { label: 'Total Pallets', value: totalPalletCount },
                { label: 'Total Khatlis', value: totalKhatliCount },
