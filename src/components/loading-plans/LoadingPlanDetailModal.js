...
import { Loader2, X, Warehouse, Calendar, User, Truck, Box, Package, Edit, Save, Trash2, AlertTriangle } from 'lucide-react';
import { format, parseISO } from 'date-fns';

// Self-contained aggregation logic for displaying pallets in the detail view
const aggregateContainerPallets = (pallets) => {
    if (!pallets || pallets.length === 0) return [];
    const tileGroups = {};
    pallets.forEach(pallet => {
        // Ensure pallet and tile exist to prevent crashes
        if (!pallet || !pallet.tile) return;
        const key = `${pallet.tile._id}-${pallet.boxCount}`;
        if (!tileGroups[key]) {
            tileGroups[key] = {
                tile: pallet.tile,
        ...
        tileGroups[key].count++;
    });
    return Object.values(tileGroups);
};
...
const LoadingPlanDetailModal = ({ planId, onClose }) => {
    ...
    // Corrected Logic: The selection modal now returns the final, complete list of pallets.
    // We simply replace the old list with this new, correct list.
        ...
        try {
            const payload = { 
                containers: editablePlan.containers.map(c => ({
                    ...
                    pallets: c.pallets.map(p => p._id) // Send only pallet IDs to the backend
                })),
    ...
    const handleDeletePlan = async () => {
        if (window.confirm('Are you sure you want to permanently delete this loading plan? This action cannot be undone and will revert all associated pallets to factory stock.')) {
            ...
            try {
                ...
                onClose(true); // Pass true to signal a refresh is needed on the parent page
            } catch (err) {
        ...
        <>
            {isPalletModalOpen && (
                <PalletSelectionModal
                    ...
                    // This is the key fix: Pass the pallets currently in the container to the modal
                    currentContainerPallets={editablePlan.containers[selectedContainerIndex].pallets}
                />
            )}

            <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 animate-fade-in" onClick={() => onClose(false)}>
                <div className="bg-foreground dark:bg-dark-foreground rounded-xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="flex justify-between items-center p-5 border-b border-border dark:border-dark-border flex-shrink-0">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <button onClick={() => onClose(false)} className="p-2 rounded-full hover:bg-background dark:hover:bg-dark-background"><X size={24} className="text-text-secondary" /></button>
                        </div>
                    </div>
                            ...
                            <div className="space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    ...
                                    <div className="p-4 bg-background dark:bg-dark-background rounded-lg">
                                        ...
                                        <div className="text-lg font-bold text-text dark:text-dark-text">{plan.createdBy?.username || 'N/A'}</div>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="space-y-4">
                                        {(isEditMode ? editablePlan.containers : plan.containers).map((container, index) => {
                                            const aggregatedPallets = aggregateContainerPallets(container.pallets);
                                            return (
                                                <div key={container._id} className="border border-border dark:border-dark-border rounded-lg">
                                                    ...
                                                    <div className="p-3">
                                                        ...
                                                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                                                            {aggregatedPallets.map((group, groupIndex) => (
                                                                <div key={groupIndex} className="p-2 bg-slate-200 dark:bg-slate-700 rounded-md text-left relative" title={`${group.tile.name}`}>
                                                                    <span className="absolute -top-2 -right-2 flex h-5 w-5 items-center justify-center rounded-full bg-primary text-white text-xs font-bold">{group.count}x</span>
                                                                    <p className="font-semibold text-sm text-text dark:text-dark-text truncate">{group.tile.name}</p>
                                                                    <p className="text-xs text-text-secondary">{group.tile.size}</p>
                                                                    <p className="text-xs text-text-secondary/70 mt-1">{group.boxCount} boxes/pallet</p>
                                                                </div>
                                                            ))}
                                                            {container.pallets.length === 0 && <p className="text-xs text-text-secondary col-span-full text-center py-4">No pallets in this container.</p>}
                                                        </div>
                                                    </div>
                                                </div>
                                            );
