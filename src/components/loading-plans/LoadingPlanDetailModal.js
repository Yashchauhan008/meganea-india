...
import { 
    Loader2, X, Warehouse, Calendar, User, Truck, Box, Package, Edit, Save, 
    Trash2, AlertTriangle, Boxes, Ruler, Layers, FileText, Clock, CheckCircle,
    XCircle, AlertCircle, ChevronDown, ChevronUp, FileSpreadsheet
} from 'lucide-react';
import { format, parseISO } from 'date-fns';
...
const LoadingPlanDetailModal = ({ planId, onClose }) => {
    ...
    const handleExportPDF = async () => {
        ...
        // Flatten all pallets from all containers
        const allItems = [];
        plan.containers?.forEach((container, cIdx) => {
            container.pallets?.forEach((pallet, pIdx) => {
                allItems.push({
                    sNo: allItems.length + 1,
                    containerNo: container.containerNumber || `Container ${cIdx + 1}`,
                    truckNo: container.truckNumber || '-',
                    palletId: pallet.palletId || '-',
                    type: pallet.type || 'Pallet',
                    tileName: pallet.tile?.name || 'Unknown',
                    tileSize: pallet.tile?.size || '-',
        ...
        await generatePDFReport({
            title: `Loading Plan - ${plan.planId}`,
            subtitle: `Factory: ${plan.factory?.name || 'N/A'} | Status: ${plan.status}`,
            headerInfo: [
                { label: 'Plan ID', value: plan.planId },
                { label: 'Factory', value: plan.factory?.name || 'N/A' },
                { label: 'Status', value: plan.status },
                { label: 'Loading Date', value: plan.loadingDate ? format(parseISO(plan.loadingDate), 'dd MMM, yyyy') : 'N/A' },
                { label: 'Created By', value: plan.createdBy?.username || 'N/A' },
                { label: 'Created Date', value: plan.createdAt ? format(parseISO(plan.createdAt), 'dd MMM, yyyy') : 'N/A' },
            ],
            summaryData: [
                { label: 'Containers', value: totals.containers, color: 'purple' },
                { label: 'Pallets', value: totals.pallets, color: 'blue' },
                { label: 'Khatlis', value: totals.khatlis, color: 'orange' },
                { label: 'Total Boxes', value: totals.boxes, color: 'green' },
            ],
            tableColumns: [
                { key: 'sNo', header: 'S.No', width: 15 },
                { key: 'containerNo', header: 'Container', width: 30 },
                { key: 'truckNo', header: 'Truck', width: 30 },
                { key: 'palletId', header: 'Pallet ID', width: 35 },
                { key: 'type', header: 'Type', width: 25 },
                { key: 'tileName', header: 'Tile', width: 45 },
                { key: 'tileSize', header: 'Size', width: 25 },
                { key: 'boxCount', header: 'Boxes', width: 25, align: 'right' },
            ],
            tableData: allItems,
            fileName: `LoadingPlan_${plan.planId}`,
    ...
    const handleExportExcel = async () => {
        ...
        await generateExcelReport({
            ...
            tableColumns: [
                ...
                { key: 'truckNo', header: 'Truck' },
                { key: 'palletId', header: 'Pallet ID' },
                { key: 'type', header: 'Type' },
                { key: 'tileName', header: 'Tile' },
                { key: 'tileSize', header: 'Size' },
                { key: 'boxCount', header: 'Boxes' },
            ],
            tableData: allItems,
            fileName: `LoadingPlan_${plan.planId}`,
            sheetName: 'Loading Plan',
        });
    };
