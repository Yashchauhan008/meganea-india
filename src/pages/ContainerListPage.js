...
import { Loader2, Search, Package, Truck, Warehouse, FileText } from 'lucide-react';
...
const ContainerListPage = () => {
  ...
  const getStatusColor = (status) => {
    const colors = {
        Loaded: 'bg-blue-100 text-blue-800 dark:bg-blue-900/40 dark:text-blue-300',
        Dispatched: 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/40 dark:text-yellow-300',
        ...
        <>
          {filteredContainers.length === 0 ? (
            <div className="text-center py-16 border-2 border-dashed border-border dark:border-dark-border rounded-lg">
              ...
              <p className="text-text-secondary mt-1 text-sm">No containers match your current filters.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {filteredContainers.map(container => {
                ...
                // --- THIS IS THE FIX ---
                // Determine the factory name. Prioritize the direct link, fall back to the plan's factory.
                ...
                return (
                  <div key={container._id} className="bg-foreground dark:bg-dark-foreground rounded-xl border border-border dark:border-dark-border shadow-sm flex flex-col transition-shadow hover:shadow-lg">
                    <div className="p-4 border-b border-border dark:border-dark-border">
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <h3 className="text-lg font-bold text-text dark:text-dark-text">{container.containerNumber}</h3>
                        </div>
                        <span className={`px-2 py-1 text-xs font-bold rounded-full ${getStatusColor(container.status)}`}>{container.status}</span>
                      </div>
                    </div>
                    ...
                    <div className="p-4 flex-grow space-y-3 text-sm">
                        ...
                        <p className="flex items-center gap-2 text-text-secondary"><Warehouse size={14}/> Factory: <span className="font-semibold text-text dark:text-dark-text">{factoryName}</span></p>
                        {/* ---------------------- */}
                    </div>
